...
### Milestone 1: Core Polling & Concurrency
...
### Milestone 2: Transactional Outbox & Resiliency
*Focus: Ensure exactly-once delivery and EF Core integration.*
- [x] **EF Core Integration:** Provide an extension method to automatically enlist message sending into the current `DbContext` transaction.
...
### Milestone 3: Developer Experience & Quality of Life
*Focus: Tooling to make local development and adoption seamless.*
- [x] **Docker / Local Dev Harness:** Scaffold a Demo App and `docker-compose.yml` for zero-friction local testing.
- [x] **XML API Docs:** Full XML documentation for IntelliSense integration and GitHub Packages README embedding.
- [ ] **Health Checks Integration:** Provide a `IHealthCheck` implementation to monitor SqlMq worker thread status and queue reachability.
- [ ] **.NET Aspire Component:** Build an Aspire integration package (`SqlMq.Hosting.Aspire`) so it effortlessly drops into modern cloud-native architectures.

### Milestone 4: Modern .NET 10 Features
...
- [ ] **High Throughput Bulk Consuming:** Process messages in bulk by allowing consumers to accept `List<T>` parameters.

### Milestone 5: Production Tooling
